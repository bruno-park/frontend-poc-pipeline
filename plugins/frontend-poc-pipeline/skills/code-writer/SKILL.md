...
**If missing, create it from the canonical implementation in [references/useUrlQuery-implementation.md](references/useUrlQuery-implementation.md)** — 전체 타입 + `string`/`number`/`boolean` 자동 감지, 배열(반복 키 `?k=a&k=b`), comma-separated serializer, schema 외 키 passthrough, default 값 URL 생략, race-safe 쓰기. `hooks/useUrlQuery.ts`에 그대로 복사한다.
