...
| 2026-06-15 | E2E "동작 검증"(action·request·response·post-action) 1급화 (Slack 피드백 반영) — #C04CWNPFFKN 김동윤 "ui도 중요하지만 action/request/response 동작이 더 중요". E2E 템플릿이 UI 가시성(`toBeVisible`·`tbody tr`·`toHaveURL`)만 단언해 요청 파라미터·본문·응답·호출 후 상태 변화 버그를 놓치던 것을 보강. (1) `e2e-test-gen/{SKILL.md, platforms/web.md}`에 **"동작 검증 원칙" 섹션 신설**(4계층 표·나쁜예/좋은예·mutation `Promise.all` 패턴·`page.route` 4xx 주입 에러 시나리오). (2) planner 위임 우선순위·generator 위임 규칙에 request/response/post-action 요구 추가. (3) 내장 폴백 템플릿 3곳(e2e-test-gen SKILL/web.md + test-writer Phase 3)을 UI-only→GET응답·search param·POST body→201→refetch+토스트·에러 동작 포함으로 교체. (4) test-writer Phase 3 위임 노트 + 합격 기준 표에 "E2E 동작 검증" 행 추가. 진입점은 `test-writer --all`(E2E는 `e2e-test-gen` 위임). 라우팅 키워드 무변경, 버전 0.7.6→0.7.7 | `plugins/.../skills/e2e-test-gen/{SKILL.md,platforms/web.md}`, `plugins/.../skills/test-writer/{SKILL.md}`, `.claude-plugin/marketplace.json`, `plugins/.../.claude-plugin/plugin.json`, `CHANGELOG.md` | E2E가 UI 검증에 치우쳐 정작 중요한 API 호출·응답·호출 후 동작을 검증하지 않던 실무 피드백을 반영 — playwright-test-* 번들 에이전트(공식 스냅샷)는 무수정, 도메인 스킬 콘텐츠만 보강 |
| 2026-06-15 | E2E 테스트 제목 컨벤션 `기능명 (라우트)` 도입 (같은 Slack 스레드 후속) — 동윤 "path도 적혀있으면 좋겠다"(첨부 대시보드 스샷=화면 이름만 노출) → 박준 "title(url) - content로 넣겠다". e2e 대시보드가 Playwright JSON `title`을 표시하므로 **최상위 describe 제목에 라우트(URL)** 를 넣어 `기능명 (라우트) - 시나리오`로 어떤 화면 테스트인지 노출. 라우트는 `const ROUTE`(planner.md URL State)에 두고 `goto`·제목 공유(drift 방지), 파일 경로 file:line은 JSON에 이미 있어 대시보드 책임이라 제목엔 라우트. `e2e-test-gen/{SKILL.md, platforms/web.md}`에 "테스트 제목 컨벤션" 섹션 + generator 위임 규칙 + 내장 템플릿 3곳(describe·beforeEach goto·RBAC goto) ROUTE 공유 교체, `test-writer` Phase 3 위임 노트 추가. 같은 0.7.7에 포함, 라우팅 키워드 무변경 | `plugins/.../skills/e2e-test-gen/{SKILL.md,platforms/web.md}`, `plugins/.../skills/test-writer/{SKILL.md}`, `CHANGELOG.md` | 대시보드 행이 화면 이름만 보여 "어느 화면 테스트인지" 식별이 어렵던 피드백 — 제목에 라우트를 박아 title(url)로 노출(파일 경로는 이미 JSON에 있어 대시보드 몫) |
| 2026-06-15 | stale 파이프라인 메타데이터 정정 (ui-builder cleanup) — (1) `marketplace.json` 플러그인 설명이 deprecated된 `ui-builder`를 활성 파이프라인 단계로 표기 + `code-writer`·`refactor`·`code-review` 누락하던 것을 실제 파이프라인(`...→ test-writer + api-integration → code-writer → refactor → code-review → create-pull-request`)으로 정정. (2) `plugin.json` 설명 `Screen Plan`(feature-planner 옛 이름)→`Feature Plan` + `Review` 단계 추가. `ui-builder` 묘비 스킬(`/ui-builder`→`code-writer --ui` 리다이렉트)·라우팅 행은 호환 위해 유지(삭제는 더 침습적). 라우팅 키워드 무변경, 버전 0.7.7→0.7.8 | `.claude-plugin/marketplace.json`, `plugins/.../.claude-plugin/plugin.json`, `CHANGELOG.md` | 배포 메타데이터가 deprecated 스킬을 활성 단계로 보여주고 실제 파이프라인과 달라 설치자가 잘못된 흐름으로 오해 — stale 참조만 정정하고 묘비 호환 경로는 보존 |
| 2026-06-15 | `unit-test-gen` 디스패처 플랫폼 라벨 러너 비종속화 — 플랫폼 표 `Web (React + Vitest)` → `Web (React, Jest/Vitest)`. `platforms/web.md`는 이미 §13 러너 비종속인데 디스패처 라벨만 옛 Vitest-only 표기로 남아 있던 잔존 stale 정정. 점검 결과 `test-writer`·`web.md`의 §13 앵커는 이미 정합(실제 drift 없음). 라우팅 키워드 무변경, 버전 0.7.8→0.7.9 | `plugins/.../skills/unit-test-gen/{SKILL.md}`, `.claude-plugin/marketplace.json`, `plugins/.../.claude-plugin/plugin.json`, `CHANGELOG.md` | 러너 비종속화(§13) 작업 당시 디스패처 라벨만 갱신 누락 — web.md 실동작(Jest/Vitest 모두)과 라벨 불일치 정리 |
| 2026-06-15 | `code-writer` 500줄 초과 → `references/` 분리 — Phase 3에 인라인된 `useUrlQuery.ts` canonical 구현 전체(~290줄)를 `references/useUrlQuery-implementation.md`로 추출, 본문엔 "없으면 reference 구현 그대로 복사" 포인터만. SKILL.md 688→393줄. 순수 구조 분리(동작·지시 무변경). 라우팅 키워드 무변경, 버전 0.7.9→0.7.10 | `plugins/.../skills/code-writer/{SKILL.md}` + `references/useUrlQuery-implementation.md`(신규), `.claude-plugin/marketplace.json`, `plugins/.../.claude-plugin/plugin.json`, `CHANGELOG.md` | 거대 코드 블록(~290줄)이 본문을 차지해 다른 Phase 지시가 묻히고 skill-architect 500줄 가이드 위반 — feature-planner와 같은 references/ 분리 패턴 적용 |
